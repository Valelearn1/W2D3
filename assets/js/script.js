...
console.table(nomi); // Per il prof: ho messo table, invece di log, per vedere meglio a livello visivo nella console. Global comment.
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  if (starWarsCharacters[i].gender === "female") {
    ...
    personaggiFemminili.push(nuovoOggetto);
  }
}
console.table(personaggiFemminili);
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  let altezza = Number(starWarsCharacters[i].height);
...
for (let i = 0; i < nomi.length; i++) {
  ...
  for (let j = 0; j < personaggiFemminili.length; j++) {
    let personaggioFemminile = personaggiFemminili[j];
